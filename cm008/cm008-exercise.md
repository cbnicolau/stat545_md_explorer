...
These functions we've seen are called **vectorized functions**. They operate element-wise in the vectors
...
-   Notice the columns that are kept. Both country and continent are kept
...
gapminder %>%
  ...
  geom_line() # ggplot doesn't recognize the grouping done by dyplyr, would need to specify the group by country in the aestetic mapping
```
...
gapminder %>%
  group_by(country) %>%
  mutate(growth = pop - first(pop)) #makes nwe column that calcs the growth in pop since first entry (yr 1952)
```
...
gapminder %>%
  group_by(country) %>%
  mutate(growth = pop - pop[year == 1972]) # [] subsets respect to a condition
```
...
gapminder %>%
  group_by(country) %>%
  mutate(change = pop - lag(pop)) # lag pop is the same pop vector shifted by one so the substraction is respect the previous yr
```
...
Similar: `lead` function. Instead of shifting the vactor forward as the lag(), it shifts the vector backwards
...
# mutate(gdp = gdpPercap*pop) %>%
#  filter(country == "Canada") %>%
#  `[[`("gdp") # extracts the vector of gdpPercap
