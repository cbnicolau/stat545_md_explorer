...
- Don't use the mouse: copying and pasting lines 19-20 in the console and it will show up in the plots viewer and then click exprot and save it
- Use `ggsave` for ggplot: this is better or reproducibility
...
p_scales + 
    scale_x_log10() +
    scale_colour_continuous(trans="log10") #scale color log 10
```
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10) #indicating the tickmarks we want to add
    ) +
    scale_y_continuous(breaks= 1:10 * 10) #add tick marks
```
...
```
 ...
 - can run `colors()` function in the console to see the color options

3. Use `RColorBrewer` to change the colour scheme.
    - Notice the three different types of scales: sequential (darkness of the color indicates), diverging (positives and negative values), and continuous.
...
p %>% 
    ggplotly() %>% 
    htmlwidgets::saveWidget("my_plotly.html")
```
