...
```r
gapminder %>% #takes gapminder dataframe
  select(year, lifeExp, country) %>% #selects the columns year, lifeExp and country (the first argument is data, but when runing a pipe it takes the result of the previous argument by default)
  arrange(year, lifeExp) #then takes the result of the previous function and arranges by year and then within the year by lifeExp
```
